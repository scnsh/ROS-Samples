...
#include <custom_rrbot_hardware_gazebo/custom_rrbot_hardware_gazebo.h>

namespace custom_rrbot_hardware_gazebo
{
  using namespace hardware_interface;

  CustomRRBotHardwareGazebo::CustomRRBotHardwareGazebo()
    : gazebo_ros_control::RobotHWSim()
  {}

  // パラメータサーバに登録されたロボットモデルから、joint名を取得
  // 取得したJoint名で、JointStateのハンドルを作成し、HardwareResourceInterfaceをインタフェースとして登録
  // 下記のhardware_interface::HardwareResourceManagerクラスのサブクラスのインスタンスを使う
  //   状態の読み出しに使う: hardware_interface::JointStateInterface jnt_state_interface_
  //   指令の書き込みに使う: hardware_interface::PositionJointInterface jnt_pos_cmd_interface_
  // 後でControllerがHardwareInterfaceを介してやりとりできるように、JointStateInterfaceやPositionJointInterfaceのハンドル作成時に、
  // jointにまつわるメンバ変数(jnt_pos_, jnt_vel_, jnt_eff_, jnt_pos_cmd_)のポインタを登録する
  bool CustomRRBotHardwareGazebo::initSim(const std::string& robot_namespace,
      ros::NodeHandle nh,
      ...
      std::vector<transmission_interface::TransmissionInfo> transmissions)
  {
    ...
    for (size_t i = 0; i < n_dof_; ++i)
    {
      ...
      jnt_eff_cmd_interface_.registerHandle(
          JointHandle(jnt_state_interface_.getHandle(jnt_names[i]), &jnt_eff_cmd_[i]));

      // ROS_DEBUG_STREAM("Registered joint '" << jnt_names[i] << "' in the PositionJointInterface.");
      ROS_DEBUG_STREAM("Registered joint '" << jnt_names[i] << "' in the EffortJointInterface.");
    }
  ...
  // 現在のGazeboにおけるロボットモデルのjointの状態(位置、速度、トルク)を取得して、
  // initSim()で渡したメンバ変数を介してHardwareInterfaceに取得した情報を教える
  void CustomRRBotHardwareGazebo::readSim(ros::Time time, ros::Duration period)
  {
    for (size_t i = 0; i < n_dof_; ++i)
    {
  ...
  // Gazeboにおけるロボットモデルのjointに指令値を送る
  // hardware_interface::PositionJointInterface jnt_pos_cmd_interface_で取得した位置指令から力指令を計算し、Gazeboに送る
  void CustomRRBotHardwareGazebo::writeSim(ros::Time time, ros::Duration period)
  {
    ...
    for (size_t i = 0; i < n_dof_; ++i)
    {
      ...
      sim_joints_[i]->SetForce(0u, jnt_eff_cmd_[i]);
    }
    // for (size_t i = 0; i < n_dof_; ++i)
    // {
    //   const double error = jnt_pos_cmd_[i] - jnt_pos_[i];
...
PLUGINLIB_EXPORT_CLASS(custom_rrbot_hardware_gazebo::CustomRRBotHardwareGazebo, gazebo_ros_control::RobotHWSim)